...
'use strict';
    const APP_VERSION='step27-home-render-scroll-copy-fix-2026-06-02-01';
    ...
    function setView(next){view=next;searchText='';cartSearch='';houseSearch='';selectedCart.clear();selectedHouse.clear();window.scrollTo(0,0);const appEl=document.getElementById('app');if(appEl)appEl.scrollTop=0;forceTopScroll();render();}
...
function render(){normalizeState();document.querySelectorAll('[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===view));const page=document.getElementById('page');
      ...
      document.getElementById('fab').style.display=['home','house','expiring','expired','terminated'].includes(view)?'none':'grid'; if(typeof updateNavBadges==='function')updateNavBadges(); renderDebug(); setSyncStatus(lastError?'error':cfg.offline?'offline':sb?'online':'config'); forceTopScroll(); setTimeout(forceTopScroll,80); setTimeout(forceTopScroll,350);
...
function renderHome(){
      const house=activeProducts().length,terminated=terminatedProducts().length,exp=expiringProducts().length,expired=expiredProducts().length,cart=cartStats().open,list=shoppingStats().open;
      return `<div class="top"><div><div class="muted">Famiglia: <b>${esc(cfg.familyCode||'offline')}</b> · <span class="sync">...</span><br><span class="version-pill">Step 27 Fix</span></div></div><div class="top-actions"><button class="icon-btn" onclick="openSettings()" title="Impostazioni">⚙️</button><button class="icon-btn logout-top" onclick="logoutFamily()" title="Esci dalla famiglia">🚪</button></div></div>
      <div class="mascot-card hero-home"><div><small>Schrodinger Fridge</small><h2>Ciao, Umano! 👋</h2><h3>Finché non apri il frigo, è contemporaneamente pieno o vuoto.</h3><p class="muted" style="color:#EDE7F8">Usa il gatto di Schrödinger per tracciare le tue riserve e sapere sempre cosa hai davvero in casa.</p></div><div class="mascot-stage"><img src="ui-home-cat.png" class="cat-loop" alt="Gatto animato"></div></div>
      <section class="hero"><small>Panoramica oggi</small><h3>Gestione famiglia</h3><div class="metrics"><div class="metric"><button onclick="setView('house')"><b>${house}</b><span>In casa</span></button></div><div class="metric"><button onclick="setView('terminated')"><b>${terminated}</b><span>Articoli terminati</span></button></div><div class="metric"><button onclick="setView('expiring')"><b>${exp}</b><span>In scadenza</span></button></div><div class="metric"><button onclick="setView('expired')"><b>${expired}</b><span>Scaduti</span></button></div></div></section>
      <div class="home-quick">${renderQuickAction('shopping','ui-lista-cat.png','Lista spesa','Da acquistare',list,'badge-shopping-home')}${renderQuickAction('cart','ui-carrello-cat.png','Lista carrello','Da sistemare',cart,'badge-cart-home')}</div>
      <div class="home-actions-ai"><button class="scan-card" type="button" onclick="openReceiptScan()"><b>Scansione scontrino</b><br><span>Foto + AI: articoli direttamente nel carrello</span></button><button class="meal-card" type="button" onclick="openMealAI()"><span class="meal-icon-wrap"><img src="ui-chef-ai-cat.png" class="meal-icon" alt="Chef AI"></span><span class="meal-copy"><b>Chef AI di casa</b><br><span>Ricette con quantità usando ciò che hai già</span></span></button></div>${renderSavedMealBanner()}
      <div class="grid home-grid">${renderCategoryTile('frigo','Frigo','Prodotti da consumare')}${renderCategoryTile('dispensa','Dispensa','Scorte di casa')}${renderCategoryTile('altro','Altro','Prodotti vari')}</div>`;
    }
