...
function renderHome(){const house=activeProducts().length,list=shoppingStats().open,exp=expiringProducts().length,expired=expiredProducts().length,cart=cartStats().open;return `<div class="top"><div><h2>Ciao, Freddie! 👋</h2><div class="muted">Famiglia: <b>${esc(cfg.familyCode||'offline')}</b> · <span class="sync">...</span></div></div><button class="icon-btn" onclick="openSettings()">⚙️</button></div><section class="hero"><small>Panoramica oggi</small><h3>Gestione famiglia</h3><div class="metrics"><div class="metric"><button onclick="setView('house')"><b>${house}</b><span>In casa</span></button></div><div class="metric"><button onclick="setView('shopping')"><b>${list}</b><span>In lista</span></button></div><div class="metric"><button onclick="setView('expiring')"><b>${exp}</b><span>In scadenza</span></button></div><div class="metric"><button onclick="setView('expired')"><b>${expired}</b><span>Scaduti</span></button></div></div></section><div class="home-quick">${renderQuickAction('shopping','ui-lista-cat.png','Lista spesa','Da acquistare',list,'badge-shopping-home')}${renderQuickAction('cart','ui-carrello-cat.png','Lista carrello','Da sistemare',cart,'badge-cart-home')}</div><div class="mascot-card"><div><small>Mascotte di casa</small><h3>Il micio controlla la cucina</h3><p class="muted" style="color:#EDE7F8">Una mascotte realistica integrata nella home, con animazione morbida in loop.</p></div><div class="mascot-stage"><img src="ui-home-cat.png" class="cat-loop" alt="Gatto animato"></div></div><div class="grid home-grid">${renderCategoryTile('frigo','Frigo','Freschi e scadenze')}${renderCategoryTile('dispensa','Dispensa','Scorte di casa')}${renderCategoryTile('altro','Altro','Prodotti vari')}</div><button class="scan-card" type="button"><b>Scansione scontrino</b><br><span>Fotocamera + AI Vision</span></button>`;}
    ...
    function statusHtml(it){if(it.checked)return '<span class="status-tag done">terminato</span>';if(isExpired(it))return '<span class="status-tag expired">scaduto</span>';if(isExpiring(it))return '<span class="status-tag expiring">in scadenza</span>';return '';}
    ...
    function renderShopping(){let items=[...state.shoppingList];const q=searchText.trim().toLowerCase();if(q)items=items.filter(x=>itemText(x).includes(q));return `<div class="top"><div><h2>Lista spesa</h2><div class="muted">${state.shoppingList.length} voci</div></div><span class="sync">...</span></div><input id="search-shopping" class="input search" placeholder="Cerca nella lista spesa" value="${esc(searchText)}" oninput="handleSearchInput('search-shopping','searchText')">${renderDuplicateAlert(state.shoppingList)}${items.length?items.map(renderShoppingItem).join(''):renderNoResults(searchText,'shopping','dispensa','Lista spesa vuota')}`;}
    function renderShoppingItem(it){return `<article class="item ${it.checked?'done':''}" onclick="toggleShoppingDone('${esc(it.id)}')"><div class="thumb">${emoji(it)}</div><div class="main"><div class="item-name">${esc(it.name)}</div><div class="meta">${fmtQty(it)}${it.notes?' · '+esc(it.notes):''}</div></div><div class="actions compact" onclick="event.stopPropagation()"><button title="Modifica" onclick="openEditModal('shopping','${esc(it.id)}')">✏️</button><button title="Sposta nel carrello" onclick="shoppingToCart('${esc(it.id)}')">🛒</button><button title="Cancella" onclick="deleteItem('shopping','${esc(it.id)}')">×</button></div></article>`;}
    function renderCart(){let items=[...state.cart];const q=cartSearch.trim().toLowerCase();if(q)items=items.filter(x=>itemText(x).includes(q));return `<div class="top"><div><h2>Lista carrello</h2><div class="muted">${state.cart.length} voci · ${selectedCart.size} selezionate</div></div><span class="sync">...</span></div><input id="search-cart" class="input search" placeholder="Cerca nel carrello" value="${esc(cartSearch)}" oninput="handleSearchInput('search-cart','cartSearch')">${renderDuplicateAlert(state.cart)}<div class="tools"><button class="ghost" onclick="selectAllCart()">Seleziona tutto</button><button class="ghost" onclick="clearCartSelection()">Deseleziona</button></div><div class="tools"><button class="primary" onclick="openMoveSelectedCart()">Sposta selezionati</button><button class="secondary" onclick="moveAllCart()">Sposta tutti</button></div><div class="tools"><button class="danger" onclick="deleteSelectedCart()">Cancella selezionati</button><button class="danger" onclick="deleteAllCart()">Cancella tutti</button></div>${items.length?items.map(renderCartItem).join(''):renderNoResults(cartSearch,'cart','dispensa','Carrello vuoto')}`;}
    function renderCartItem(it){const on=selectedCart.has(it.id);return `<article class="item compact ${it.checked?'done':''}" onclick="toggleCartDone('${esc(it.id)}')"><button class="check-btn ${on?'on':''}" onclick="event.stopPropagation();toggleCartSelect('${esc(it.id)}')">${on?'✓':''}</button><div class="main"><div class="item-name">${esc(it.name)}</div><div class="meta">${fmtQty(it)} · ${catLabel(it.category)}${it.expiry?' · scad. '+esc(it.expiry):''}${it.notes?' · '+esc(it.notes):''}</div></div><div class="actions compact" onclick="event.stopPropagation()"><button title="Modifica" onclick="openEditModal('cart','${esc(it.id)}')">✏️</button><button title="Sposta in ${catLabel(it.category)}" onclick="moveCartItemToProduct('${esc(it.id)}')">➡️</button><button title="Cancella" onclick="deleteItem('cart','${esc(it.id)}')">×</button></div></article>`;}
    ...
    function emoji(it){const src=it.list_type==='shopping'?'ui-lista-cat.png':it.list_type==='cart'?'ui-carrello-cat.png':iconFor(it.category);return `<img src="${src}" alt="" class="thumb-icon">`;}
    ...
    function selectAllCart(){state.cart.forEach(x=>selectedCart.add(x.id));render();}
    ...
    async function deleteSelectedCart(){await deleteMany('cart',[...selectedCart]);selectedCart.clear();}
    ...
    function houseBaseForMode(mode){let arr=[...state.products];if(mode==='expiring')arr=arr.filter(isExpiring);if(mode==='expired')arr=arr.filter(isExpired);return arr;}
    function selectAllHouse(mode='all'){houseBaseForMode(mode).forEach(x=>selectedHouse.add(x.id));render();}
    ...
    async function deleteSelectedHouse(){await deleteMany('product',[...selectedHouse]);selectedHouse.clear();}
